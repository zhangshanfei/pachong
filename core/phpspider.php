...
class phpspider
{
     ...
     * @param mixed $url
     * @param mixed $link
     * @return string $html  //zsf 应该是页面内容，字符串
     * @author seatle <seatle@foxmail.com> 
     * @created time :2016-09-18 10:17
     */
     ...
     * 根据配置提取HTML代码块中的字段
     * 
     * @param mixed $confs		//configs[]中的fields
     * @param mixed $html
     * @param mixed $page
     * @return void
     * @author seatle <seatle@foxmail.com> 
    ...
    public function get_fields($confs, $html, $url, $page) 
    {
        ...
        foreach ($confs as $conf) 
        {
            ...
            if (!empty($conf['selector'])) 
            {
                ...
                {
                	// zsf 测试到此处的html代码仍然是正常的。
                	// $values = $html;
                	// 也就是下面这个get_fields_xpath方法在处理$html的时候出了问题,对于特殊字符<会导致loadHtml()加载html时出错
     ...
     * 采用xpath分析提取字段
     * 
     * @param mixed $html		页面内容
     * @param mixed $selector	xpath选择器字符串
     * @return mixed $nodeValues	获取字段内容
     * @author seatle <seatle@foxmail.com> 
     * @created time :2016-09-18 10:17
     */
...
//         return $result;
		   ...
		   $result = $crawler->filter($selector)->html();
